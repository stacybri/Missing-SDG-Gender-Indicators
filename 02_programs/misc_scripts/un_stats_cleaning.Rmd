...
un_pull <- function(series, start, end) {
  ...
  read_csv(paste(raw_dir, "/sdg_data/raw/", series, "_raw.csv", sep = "")) %>%
    ...
    group_by(iso3c, date) %>%
    summarise(
      ind_value = max(as.numeric(Value), na.rm = T),
      ind_metadata = first(Nature),
      ind_quality = max(quality, na.rm = T)
    )
}
```
