---
title: 'Reproducibility Package for "Missing SDG Gender Indicators: 2016-2023"'
  ...
  any <- un_sdg_filled_df %>%
    ...
    tier1 <- un_sdg_combined_df %>%
      ...
      summarise(ind_quality=max(ind_quality, na.rm=T),
        ...
        mutate(ind_quality=if_else(is.na(ind_quality),0,ind_quality)) %>% # if the indicator is missing for a year in the database, set availability to 0.
        group_by(iso3c) %>%
        summarise(ind_quality=round(sum(ind_quality)/30,3),
                goal_count=30,
                ind_number=n() #get whether indicator exists at all in 5 years of this across countries
                ) %>%
      left_join(country_metadata) %>%
    ...
    tier2 <- un_sdg_combined_df %>%
      ...
      summarise(ind_quality=max(ind_quality, na.rm=T),
        ...
        mutate(ind_quality=if_else(is.na(ind_quality),0,ind_quality)) %>% # if the indicator is missing for a year in the database, set availability to 0.
        group_by(iso3c) %>%
        summarise(ind_quality=round(sum(ind_quality)/20,3),
                goal_count=20,
                ind_number=n() #get whether indicator exists at all in 5 years of this across countries
                ) %>%
      left_join(country_metadata) %>%
...
all_sdg_df <- read_csv(paste0(raw_dir, "/SPI_D3_UNSD_alltier_data_",window,"yr.csv")) %>%
...
# get totals for all SDGs
all_gender_sdg_df <- read_csv(paste0(raw_dir, "/SPI_D3_UNSD_gender_data_",window,"yr.csv")) %>%
  filter(date==upper_year) %>%
  filter(!(iso3c %in% pop_exlude_cntry)) %>%
    #collapse to country level
...
# get totals for all SDGs
all_nongender_sdg_df <- read_csv(paste0(raw_dir, "/SPI_D3_UNSD_nongender_data_",window,"yr.csv")) %>%
  filter(date==upper_year) %>%
  filter(!(iso3c %in% pop_exlude_cntry)) %>%
    #collapse to country level
  group_by(iso3c, country, goal) %>%
...
avail_SPI_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_',window,'yr.csv')) %>%   filter(!(iso3c %in% pop_exlude_cntry)) %>%
...
avail_plt_df5 <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_',window,'yr.csv')) %>%   filter(!(iso3c %in% pop_exlude_cntry)) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
...
any_avail_plt_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_',window,'yr.csv')) %>%
  filter(!(iso3c %in% pop_exlude_cntry)) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
...
avail_plt_micsdhs_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_',window,'yr.csv')) %>%
  filter(!(iso3c %in% pop_exlude_cntry)) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
...
avail_plt_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_',window,'yr.csv')) %>%   filter(!(iso3c %in% pop_exlude_cntry)) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
