...
#all list
all_list_df <- jsonlite::fromJSON(indicators_url, flatten = TRUE) %>%
  as_tibble() %>%
...
all_sdg_df <- read_csv(paste0(raw_dir, "/SPI_D3_UNSD_alltier_data_5yr.csv")) %>%
  ...
  summarise(
    ...
    total_indicators=sum(ind_number)
  ) 
...
```{r}
avail_top5_dhs_SPI_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_5yr.csv')) %>%   filter(!(iso3c %in% pop_exlude_cntry)) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
  filter(date==2019 & goal=="Top 5 DHS/MICS Dropped") #keep just most recent year

#calculate global average
world_avg <-
  avail_top5_dhs_SPI_df %>%
  summarise(avg = mean(availability, na.rm = T)) %>%
  pull(avg)

world_p25 <- scales::comma(quantile(avail_top5_dhs_SPI_df$availability, p=0.25, na.rm=TRUE))
world_p50 <- scales::comma(quantile(avail_top5_dhs_SPI_df$availability, p=0.5, na.rm=TRUE))
world_p75 <- scales::comma(quantile(avail_top5_dhs_SPI_df$availability, p=0.75, na.rm=TRUE))
...
avail_top5_dhs_SPI_df <- avail_top5_dhs_SPI_df %>%
...
avail_top5_dhs_SPI_df %>%
  ...
  #stat_summary(fun = mean, geom = "point", size = 5, color='black') +
    geom_text(data=inc_avg, aes(y=avg, x=income_level, label=scales::comma(round(avg,0))), color="black", position=position_dodge()) +
  ...
  ylab('SDG Gender Indicator Availability') +
   scale_x_discrete(
     limits=levels(avail_top5_dhs_SPI_df$income_level),
     breaks=levels(avail_top5_dhs_SPI_df$income_level)
   ) +
  # scale_y_log10(
  #   labels=scales::comma
  # ) +
