...
pop_exlude_cntry <- WDI_df %>% filter(date==2023) %>% filter(SP.POP.TOTL <= pop_filt) %>% select(iso3c) %>% pluck(1)
pop_exlude_cntry_names <- WDI_df %>% left_join(wb_countries) %>% filter(date==2023) %>% filter(SP.POP.TOTL <= pop_filt) %>% select(country) %>% pluck(1)
...
tier1_list <- tier1_list$series_code

# get list of tier 2 indicaors
tier2_list <- list_df %>%
  filter(tier==2)

tier2_list <- tier2_list$series_code
...
table1dat <- list_df %>%
   transmute(
    ...
    Tier=tier,
    `Sub-Description`=series_description,
    Gender_Breakdown=Gender_Breakdown
  ) %>%
  #replace Description with Sub-Description if ccode==1110 and 1111
  #mutate(Description=if_else(ccode %in% c(1110,1111), `Sub-Description`, Description)) %>%
...
for (series in list_df$series_code) {
  ...
  if (file.exists(paste(raw_dir,'/sdg_data/',series,'.csv',sep=""))) {
    temp <- read_csv(paste(raw_dir,'/sdg_data/',series,'.csv',sep=""), show_col_types=FALSE) %>%
      mutate(
        ...
        series_description=as.character(series_description)
      )
...
for (series in list_df$series_code) {
  ...
  if (file.exists(paste(raw_dir,'/sdg_data/',series,'.csv',sep=""))) {
    ...
    tempmeta <- read_csv(paste(raw_dir,'/sdg_data/',series,'.csv',sep=""),show_col_types=FALSE) %>%
    filter(!is.na(ind_value)) %>%
    group_by(ind_metadata, series_code) %>%
    summarise(n = n()) %>%
    ungroup() %>%
    mutate(tot = sum(n),
 ...
 un_sdg_meta_wide <- un_sdg_meta %>%
  filter(!is.na(ind_metadata)) %>%
   group_by(series_code, ind_metadata) %>%
  ...
  mutate(source_share = paste0("C: (", C, "), CA: (", CA, "), E: (", E, "), G: (", G, "), M: (", M, ")"),
   ...
   select(series_code, source_share)
...
for (series in list_df$series_code) {
  ...
  if (file.exists(paste(raw_dir,'/sdg_data/',series,'_sex_disagg.csv',sep=""))) {
    temp <- read_csv(paste(raw_dir,'/sdg_data/',series,'_sex_disagg.csv',sep=""),show_col_types=FALSE) %>%
      mutate(
        date=as.numeric(date), 
        goal=as.character(goal), 
        target=as.character(target), 
...
gender_sdg_df_empty <- map_dfr(list_df$series_code, ~(gender_df_empty %>% mutate(series_code=.x)))
...
gender_disagg_sdg_df_empty <- map_dfr(list_df$series_code, ~(gender_disagg_empty %>% mutate(series_code=.x)))
...
#create database of gender indicators whether they exist at all
  un_sdg_filled_df <-  gender_sdg_df_empty %>%
    left_join(sdg_tab_merge %>% filter(!is.na(series_code))) %>% #add ccode for specialized indicator grouping
    left_join(un_sdg_df) %>%
...
un_aki_fun <- function(date_start, date_end) {
  ...
  global <- un_sdg_combined_df %>%
    ...
    summarise(ind_quality=max(ind_quality, na.rm=T),
      ...
      group_by(iso3c) %>%
      summarise(ind_total=sum(ceiling(ind_quality), na.rm=TRUE),
                ind_quality=round(ind_total/50,3),
                goal_count=50,
                ind_number=n() #get whether indicator exists at all in 5 years of this across countries
              ) %>%
    left_join(country_metadata) %>%
    filter(!is.na(region)) %>%
    select(iso3c, country,region, income_level, ind_total, ind_quality,ind_number,goal_count) %>%
    mutate(date=date_end,
           goal="Combined")

  any <- un_sdg_filled_df %>%
    filter(between(date,date_start,date_end) ) %>%
    filter(!is.na(ind_quality)) %>%
    group_by(iso3c, ccode) %>%
    summarise(ind_quality=max(ind_quality, na.rm=T),
    ...
    select(iso3c, country,region, income_level, ind_total, ind_quality,ind_number,goal_count) %>%
    mutate(date=date_end,
           goal="Any Data Available")
    
    tier1 <- un_sdg_combined_df %>%
      filter(between(date,date_start,date_end) ) %>%
      filter(!is.na(ind_quality)) %>%
      filter(series_code %in% tier1_list) %>% #get just tier1 indicators  
      group_by(iso3c, ccode) %>%
      summarise(ind_quality=max(ind_quality, na.rm=T),
                ind_value=mean(ind_value, na.rm=T),
                ind_metadata=first(ind_metadata)) %>% #check if any values (even sub-indicators) for indicator
        ...
        group_by(iso3c) %>%
        summarise(ind_total=sum(ceiling(ind_quality), na.rm=TRUE),
                ind_quality=round(ind_total/30,3),
                goal_count=30,
                ind_number=n() #get whether indicator exists at all in 5 years of this across countries
                ) %>%
      left_join(country_metadata) %>%
    ...
    tier2 <- un_sdg_combined_df %>%
      filter(between(date,date_start,date_end) ) %>%
      filter(!is.na(ind_quality)) %>%
      filter(series_code %in% tier2_list) %>% #get just tier1 indicators  
      group_by(iso3c, ccode) %>%
      summarise(ind_quality=max(ind_quality, na.rm=T),
                ind_value=mean(ind_value, na.rm=T),
                ind_metadata=first(ind_metadata)) %>% #check if any values (even sub-indicators) for indicator
        ...
        group_by(iso3c) %>%
        summarise(ind_total=sum(ceiling(ind_quality), na.rm=TRUE),
                  ind_quality=round(ind_total/20,3),
                goal_count=20,
                ind_number=n() #get whether indicator exists at all in 5 years of this across countries
                ) %>%
      left_join(country_metadata) %>%
    ...
    top5_dhs_mics <- un_sdg_combined_df %>%
      filter(between(date,date_start,date_end) ) %>%
      filter(!is.na(ind_quality)) %>%
      filter(!(series_code %in% top5_dhs_mics_list)) %>% #get just tier1 indicators  
      group_by(iso3c, ccode) %>%
      summarise(ind_quality=max(ind_quality, na.rm=T),
                ind_value=mean(ind_value, na.rm=T),
                ind_metadata=first(ind_metadata)) %>% #check if any values (even sub-indicators) for indicator
        ...
        group_by(iso3c) %>%
        summarise(ind_total=sum(ceiling(ind_quality), na.rm=TRUE),
                  ind_quality=round(ind_total/45,3),
                goal_count=45,
                ind_number=n() #get whether indicator exists at all in 5 years of this across countries
                ) %>%
      left_join(country_metadata) %>%
    ...
    top7_dhs_mics <- un_sdg_combined_df %>%
      filter(between(date,date_start,date_end) ) %>%
      filter(!is.na(ind_quality)) %>%
      filter(!(series_code %in% top7_dhs_mics_list)) %>% #get just tier1 indicators  
      group_by(iso3c, ccode) %>%
      summarise(ind_quality=max(ind_quality, na.rm=T),
                ind_value=mean(ind_value, na.rm=T),
                ind_metadata=first(ind_metadata)) %>% #check if any values (even sub-indicators) for indicator
        ...
        group_by(iso3c) %>%
        summarise(ind_total=sum(ceiling(ind_quality), na.rm=TRUE),
                  ind_quality=round(ind_total/43,3),
                goal_count=43,
                ind_number=n() #get whether indicator exists at all in 5 years of this across countries
                ) %>%
      left_join(country_metadata) %>%
  ...
  goal <- un_sdg_combined_df %>%
    ...
    group_by(iso3c,goal) %>%
    summarise(ind_total=sum(ceiling(ind_quality), na.rm=TRUE),
              ind_quality=round(ind_total/mean(goal_count),3),
              goal_count=mean(goal_count),
              ind_number=n() #get whether indicator exists at all in 5 years of this across countries
              ) %>%
    left_join(country_metadata) %>%
    filter(!is.na(region)) %>%
    select(iso3c, country,region, income_level,goal, ind_total, ind_quality,ind_number, goal_count) %>%
    mutate(date=date_end
    ) 
  ...
  global %>%
    ...
    select(iso3c,date, country,region, income_level,goal, ind_total, ind_quality, ind_number,goal_count)
...
# 10 Year window
####
#create this database for each year from 2004 to 2023 using a 5 year average
for (i in c(2009:2023)) {
...
```{r}
# produce a dataset with indicator counts by goal
avail_counts_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_5yr.csv')) %>%   filter(!(iso3c %in% pop_exlude_cntry)) %>%
  filter(date==2023) %>%
  select(iso3c,date,country, region, income_level, goal, ind_total) %>%
  #reshap wide based on goal. This will give us a count of indicators by goal
  pivot_wider(names_from=goal, values_from=ind_total, names_prefix='Sex Disaggregated Data Available: ') %>%
  rename(
    `Sex Disaggregated Data Available: 50 Gender SDG Indicators`=`Sex Disaggregated Data Available: Combined`,
    `Any Data Available: 50 Gender SDG Indicators`=`Sex Disaggregated Data Available: Any Data Available`,
  )

write_excel_csv(avail_counts_df, paste0(output_dir, '/SPI_Gender_SDG_indicator_counts.csv'))
```
...
avail_SPI_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_5yr.csv')) %>%   filter(!(iso3c %in% pop_exlude_cntry)) %>%
  ...
  filter(date==2022 & goal=="Combined") #keep just most recent year
...
gii_df <- read_csv(paste0(raw_dir, "/misc/HDR23-24_Composite_indices_complete_time_series.csv")) %>%
  transmute(iso3c=iso3,
         gii=gii_2022)

sigi_df <- read_csv(paste0(raw_dir, "/misc/OECD.DEV.NPG,DSD_SIGI@DF_SIGI_2023,1.0+all.csv")) %>%
  filter(Measure=="SIGI score") %>%
  transmute(iso3c=REF_AREA,
            sigi=OBS_VALUE)

#read in ODIN data
odin_df <- read_excel(path=paste0(raw_dir, "/misc/ODIN_Gender_2022.xlsx")) %>%
...
combined_df_sv <- combined_df %>%
  select(iso3c, country, region, income_level, IQ.SPI.OVRL, availability, IQ.SPI.PIL3, NY.GDP.PCAP.PP.CD, SP.POP.TOTL,HD.HCI.OVRL,GE.EST,SG.LAW.INDX,gii,sigi,NY.GDP.PCAP.KD, OGDI, ODIN) %>%
...
avail_plt_df5 <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_5yr.csv')) %>%   filter(!(iso3c %in% pop_exlude_cntry)) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
  filter(date==2023 & goal=="5") #keep just most recent year
...
avail_plt_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_5yr.csv')) %>%
  filter(!(iso3c %in% pop_exlude_cntry)) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
  filter(date==2023 & goal=="Combined") %>% #keep just most recent year
  left_join(WDI_df) %>%
...
#calculate share of global population (SP.POP.TOTL) with less than 50% availability
pop_share <- avail_plt_df %>%
  ungroup() %>%
  mutate(pop_total=sum(SP.POP.TOTL, na.rm=TRUE)) %>%
  filter(availability<50) %>%
  summarise(pop_share=sum(SP.POP.TOTL, na.rm=TRUE)/pop_total) 
...
avail_plt_df %>%
  ...
  geom_jitter(aes(size = SP.POP.TOTL), alpha = 0.25, width = 0.2, height=0) +
...
```{r}
#availability if all sex disaggregated indicators had value where overall value was present
any_avail_plt_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_5yr.csv')) %>%
  filter(!(iso3c %in% pop_exlude_cntry)) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
  filter(date==2023 & goal=="Any Data Available") 
...
For example, of the 32 gender-related indicators which are sex disaggregated, if countries which had a population estimate also had a sex-disaggregated estimate (almost always, if not always, feasible), the SDG gender coverage rate would be `r 100*round(any_world_avg,3)`% instead of `r 100*round(world_avg,3)`%. 
...
```{r}
#add mics/dhs comparison

avail_plt_micsdhs_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_5yr.csv')) %>%
  filter(!(iso3c %in% pop_exlude_cntry)) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
  filter(date==2023 & goal=="Top 5 DHS/MICS Dropped") %>% #keep just most recent year
  left_join(WDI_df) %>%
  mutate(income_alt_groups=case_when(
    income_level=="High income" & SP.POP.TOTL<=500000 ~ "Hgh Income & Pop <= 500K",
    income_level=="High income" & SP.POP.TOTL>500000 ~ "Hgh Income & Pop >= 500K",
...
sdg_121 <- un_sdg_df %>%
  filter(series_code=="SI_POV_NAHC") %>%
  filter(!(iso3c %in% pop_exlude_cntry)) %>%
  filter(date>=2016) %>%
  group_by(iso3c) %>%
  summarise(
    `Number of Data Points`=mean(ind_quality)*8
  ) %>%
  filter(`Number of Data Points`>0) %>%
  left_join(country_metadata)
```
...
metainfo <- list_df %>% 
  left_join(sdg_tab_merge %>% filter(!is.na(series_code))) %>%
...
table1_df <- disagg_tab_df %>%
  ...
  select(Goal, Indicator, Tier, Description, everything()) %>%
  mutate(Description=if_else(ccode %in% c(1110,1111), `Sub-Indicators`, Description)) %>%
  select(-ccode, - Gender_Breakdown, -`Sub-Indicators`) %>%
...
```{r}
#compare values in table1_df with original
table_1_orig <- read_csv(paste0(output_dir,"/table_1_paper_version.csv" )) %>%
  filter(!is.na(Indicator)) %>%
  mutate(across(c("Sex Disaggregation Available", "Any Data Available"), ~as.numeric(str_remove(., "%")))) %>%
  transmute(
    Description=Description,
    `Sub-Indicators`=`Sub-Indicator (if any)`,
    `Sex Disaggregation Available - Original`= `Sex Disaggregation Available`,
    `Any Data Available - Original`= `Any Data Available`)

#join the tables and save as csv to output_dir
table1_comb_df <- table1_df %>%
  rename(
    `Sex Disaggregation Available - Updated`=`Sex Disaggregation Available`,
    `Any Data Available - Updated`=`Any Data Available`
  )
table1_comb_df$`Sex Disaggregation Available - Original` <- table_1_orig$`Sex Disaggregation Available - Original`
table1_comb_df$`Any Data Available - Original` <- table_1_orig$`Any Data Available - Original`

#create a flag if the values are different by more than 5 points
table1_comb_df$`Sex Disaggregation Available - Different` <- ifelse(abs(table1_comb_df$`Sex Disaggregation Available - Updated`-table1_comb_df$`Sex Disaggregation Available - Original`)>5,"More than 5 point change","little change")
#now more than 10 points
table1_comb_df$`Sex Disaggregation Available - Different` <- ifelse(abs(table1_comb_df$`Sex Disaggregation Available - Updated`-table1_comb_df$`Sex Disaggregation Available - Original`)>10,"More than 10 point change",table1_comb_df$`Sex Disaggregation Available - Different`)

#now more than 20 points
table1_comb_df$`Sex Disaggregation Available - Different` <- ifelse(abs(table1_comb_df$`Sex Disaggregation Available - Updated`-table1_comb_df$`Sex Disaggregation Available - Original`)>20,"More than 20 point change",table1_comb_df$`Sex Disaggregation Available - Different`)

#do same for any data available
#more than 5 points
table1_comb_df$`Any Data Available - Different` <- ifelse(abs(table1_comb_df$`Any Data Available - Updated`-table1_comb_df$`Any Data Available - Original`)>5,"More than 5 point change","little change")
#now more than 10 points
table1_comb_df$`Any Data Available - Different` <- ifelse(abs(table1_comb_df$`Any Data Available - Updated`-table1_comb_df$`Any Data Available - Original`)>10,"More than 10 point change",table1_comb_df$`Any Data Available - Different`)
#now more than 20 points
table1_comb_df$`Any Data Available - Different` <- ifelse(abs(table1_comb_df$`Any Data Available - Updated`-table1_comb_df$`Any Data Available - Original`)>20,"More than 20 point change",table1_comb_df$`Any Data Available - Different`)


#write to csv
write_csv(table1_comb_df, paste0(output_dir, "/table1_comparison.csv"))
...
```{r}
data <- 'combined_df'
indicators <- c("availability")
reference_year <- 2022

      df_overall <- get(data) %>%
      filter(date==as.numeric(reference_year)) %>% 
      rename(SPI.INDEX=IQ.SPI.OVRL) %>%
      #muttiply availability by 50 to get number rather than share
      mutate(availability=availability*50/100) %>%
...
#get regional aggregates for SPI pillar 3
reg_spi_df <- combined_df %>%
  group_by(region) %>%
  summarise(
    IQ.SPI.PIL3=mean(IQ.SPI.PIL3, na.rm=T),
    availability=mean(availability, na.rm=T)
  ) %>%
  ungroup() %>%
  mutate(region=factor(region, levels=unique(combined_df$region)))

#get income aggregates as well
inc_spi_df <- combined_df %>%
  group_by(income_level) %>%
  summarise(
    IQ.SPI.PIL3=mean(IQ.SPI.PIL3, na.rm=T),
    availability=mean(availability, na.rm=T)
  ...
  ungroup() %>%
  mutate(income_level=factor(income_level, levels=unique(combined_df$income_level)))

#global
global_spi_df <- combined_df %>%
  summarise(
    IQ.SPI.PIL3=mean(IQ.SPI.PIL3, na.rm=T),
    availability=mean(availability, na.rm=T)
  ) %>%
...
gender_overperformers_plot + 
  plot_annotation(
    # title='Top 15 Over/Under-Performers on SPI overall score compared to GDP per capita and Human Capital Index',
                  caption=str_wrap('Over and under performers calculated for Availability of Gender Indicators by using OLS regression of Availability on SPI overall score in 2022 and calculting residuals from this regression.  ',100)
                  )
```
...
'GDP Only' = feols(availability ~  log(NY.GDP.PCAP.PP.CD)  , data=avail_wbstats_reg_df, se='hetero'),
...
avail_plt_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_5yr.csv')) %>%   filter(!(iso3c %in% pop_exlude_cntry)) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
  filter(date==2023 & goal=="Tier 1") #keep just most recent year
```
...
avail_plt_df %>%
  ...
  #geom_boxplot(aes(color=region), outlier.alpha = 0) +
  geom_jitter(size = 2, alpha = 0.25, height = 0) +
  #geom_point(size = 3, alpha = 0.15) +
  #stat_summary(fun = mean, geom = "point", size = 5, color='black') +
  ...
  annotate(
    "text", x = 6.5, y = 65, family = "Poppins", size = 4, color = "gray20",
...
#                                                       "Lower middle income", "Low income"))) %>%
  ...
  #geom_boxplot(aes(color=region), outlier.alpha = 0) +
  geom_jitter(size = 2, alpha = 0.25, width = 0.2, height=0) +
  #geom_point(size = 3, alpha = 0.15) +
  #stat_summary(fun = mean, geom = "point", size = 5, color='black') +
  geom_text(data=inc_avg, aes(y=avg, x=income_level, label=scales::comma(round(avg,1))), color="black", position=position_dodge()) +
  #geom_text(data=reg_avg, aes(y=avg, x=region, label=scales::comma(round(avg,1))), color="black") +
...
avail_plt_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_5yr.csv')) %>%   filter(!(iso3c %in% pop_exlude_cntry)) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
  filter(date==2023 & goal=="Tier 2") #keep just most recent year
```
...
sources_explained <- sources_categorized_df %>%
  group_by(goal, data_type, code, series_code, description, series_description) %>%
  summarise(n=sum(n),
            `Example Source`=first(Source))  %>% 
    group_by(goal, code, series_code, description, series_description) %>%
     ...
     summarise(code = first(code),
               ...
               description1 = first(series_description),
               datatype_con = first(datatype_con),
               `Example Source` = first(`Example Source`)) %>%
  left_join(un_sdg_meta_wide, by = "series_code") %>%
  arrange(as.numeric(goal), code) 

  
sources_explained %>%
  select(goal, code, series_code, description, datatype_con, source_share, `Example Source`) %>%
  rename('Goal' = 'goal',
         'Indicator' = 'code',
         'Code' = 'series_code',
         'Description' = 'description',
